...
public:
	...
	Buffer<float> ssaoBuffer;           // Half-resolution AO
	Buffer<float> ssaoBlurBuffer;       // Half-resolution, edge-aware filtered AO
	
	// SSAO/SSGI settings
	static constexpr int SSAO_SAMPLES = 16;
	static constexpr float SSAO_RADIUS = 0.2f;
	static constexpr float SSAO_BIAS = 0.005f;
	static constexpr int SSAO_DOWNSAMPLE = 2;
	...
	float ssgiIntensity = 0.0f;
	float ambientIntensity = 0.1f;

	// Cached once per frame. Avoid rebuilding camera matrices for every AO/GI sample.
	glm::mat4 frameViewMatrix{1.0f};
	glm::mat4 frameProjectionMatrix{1.0f};
	glm::mat4 frameViewProjectionMatrix{1.0f};
...
private:
	...
	float computeSSAO(int x, int y) const;
	glm::vec3 computeSSGI(int x, int y) const;
	glm::vec3 getRandomVector(int x, int y) const;
	void blurSSAO();
	float sampleSSAO(int x, int y) const;
...
public:
	...
	void renderWithSSAO(Scene& scene);
